...
export const BarChart: React.FC<BarChartProps> = ({ data, color = COLORS.accent, height = 50 }) => {
  ...
  const maxFromDatum = data.map((d) => (typeof d === 'number' ? undefined : d.maxValue)).filter(Boolean) as number[];
  ...
  return (
    <svg width={w} height={height}>
      {values.map((v, i) => {
        ...
        return <rect key={i} x={i * (bw + gap)} y={height - bh} width={bw} height={bh} fill={fill} rx={2} opacity={0.7 + (i / values.length) * 0.3} style={{ filter: `drop-shadow(0 0 4px ${fill})` }} />;
      })}
    </svg>
  );
};
...
interface DonutProps { segments: DonutSegment[]; }
