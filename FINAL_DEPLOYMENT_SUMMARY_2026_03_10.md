...
1. Credential Flow Hardening
   - ✅ Updated `scripts/vault/sync_gsm_to_vault.sh` to prefer VAULT_TOKEN_FILE, AppRole fallback
   - ✅ Updated `backend/src/credentials.ts` with 4-layer resolver (GSM → Vault → KMS → cache)
   - ✅ Updated `backend/Dockerfile.prod` to set VAULT_TOKEN_FILE=/var/run/secrets/vault/token
   - ✅ Updated `backend/docker-entrypoint.sh` to load VAULT_TOKEN from file and start Vault Agent
...
6. Local Validation (Mock GSM → Vault Flow)
   ...
   - ✅ Token-file selection logic verified (prefers file over env)
...
Summary Documents:
  ...
  - FINAL_DEPLOYMENT_SUMMARY_2026_03_10.md (this file, in repo root)
