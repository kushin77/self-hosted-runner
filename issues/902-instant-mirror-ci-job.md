...
- [x] Wire `ci_templates/mirror-to-github.yml` into `config/cicd/.gitlab-ci.yml` or include it as a local template.
- [x] Configure `GITHUB_MIRROR_SSH_KEY` as protected+masked and create a GitLab pipeline schedule or MR-protected manual job for the mirror. Use `scripts/ci/gitlab_set_variable.sh` to automate setting protected variables via API (requires `GITLAB_API_TOKEN`).
 - [x] Added `scripts/ci/rotate_github_deploy_key.sh` to automate generating a new deploy key, adding it to GitHub, and storing the private key in GitLab CI variables. Use with `GITHUB_TOKEN`, `GITLAB_API_TOKEN`, and `GROUP_ID` or `PROJECT_ID`. (Implemented)
