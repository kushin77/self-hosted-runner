...
server.on('error', (err) => {
  if (err.code === 'EADDRINUSE') {
    console.error(`ERROR: Port ${PORT} is already in use on host ${HOST}`);
    console.error('This usually indicates a port mismatch or the mock server is already running.');
    process.exit(1);
  } else if (err.code === 'EADDRNOTAVAIL') {
    console.error(`ERROR: Cannot bind to address ${HOST}:${PORT} - host address not available`);
    console.error('Ensure WORKER_HOST is set to a valid hostname or IP address.');
    process.exit(1);
  } else {
