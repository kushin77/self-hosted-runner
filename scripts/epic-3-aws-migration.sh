...
# Source retry helper if present
...
check_aws_prerequisites() {
  ...
  if ! retry_cmd ${TRAFFIC_RETRY_ATTEMPTS:-3} 1 aws sts get-caller-identity &>/dev/null; then
...
phase_live_failover() {
  ...
  if [ "$DRY_RUN" = "false" ]; then
    if retry_cmd ${TRAFFIC_RETRY_ATTEMPTS:-3} 2 aws elbv2 modify-listener --listener-arn arn:aws:elasticloadbalancing:${AWS_REGION} &>/dev/null; then
      log_event "aws_failover" "success" "10% traffic shifted to AWS"
    else
      log_event "aws_failover" "warning" "10% traffic shift failed (transient), will retry next stage"
    fi
  else
  ...
  if [ "$DRY_RUN" = "false" ]; then
    if retry_cmd ${TRAFFIC_RETRY_ATTEMPTS:-3} 2 aws elbv2 modify-listener --listener-arn arn:aws:elasticloadbalancing:${AWS_REGION} &>/dev/null; then
      ...
      log_event "aws_failover" "warning" "50% traffic shift failed (transient), retrying"
    fi
  else
  ...
  if [ "$DRY_RUN" = "false" ]; then
    if retry_cmd ${TRAFFIC_RETRY_ATTEMPTS:-3} 2 aws elbv2 modify-listener --listener-arn arn:aws:elasticloadbalancing:${AWS_REGION} &>/dev/null; then
      ...
      log_event "aws_failover" "warning" "90% traffic shift failed (transient), retrying"
    fi
  else
  ...
  if [ "$DRY_RUN" = "false" ]; then
    if retry_cmd ${TRAFFIC_RETRY_ATTEMPTS:-3} 2 aws elbv2 modify-listener --listener-arn arn:aws:elasticloadbalancing:${AWS_REGION} &>/dev/null; then
      ...
      log_event "aws_failover" "failure" "100% traffic shift failed after retries"
      return 1
    fi
  else
