...
# - If VAULT_TOKEN_FILE is present, read token from that file (vault-agent sink).
...
VAULT_TOKEN="$vault_token" vault kv put "${VAULT_KV_MOUNT}/${kv_path}" value="$(echo "$secret_payload" | base64 -w0)" >/dev/null 2>&1
rc=$?
