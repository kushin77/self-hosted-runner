...
#   1. Vault (VAULT_ADDR + REDACTED_VAULT_TKN)
...
auto_detect_provider() {
  ...
  if [[ -n "${VAULT_ADDR:-}" && -n "${REDACTED_VAULT_TKN:-}" ]]; then
  ...
  case "${CURRENT_SOURCE,,}" in
    vault)
      if check_vault 2>/dev/null; then
        ...
        (cd "$REPO_ROOT" && GITHUB_ISSUE_ID="$GITHUB_ISSUE_ID" ./scripts/direct-deploy.sh vault "$TARGET_BRANCH") && exit 0 || {
    ...
    aws)
      if check_aws 2>/dev/null; then
        ...
        (cd "$REPO_ROOT" && GITHUB_ISSUE_ID="$GITHUB_ISSUE_ID" ./scripts/direct-deploy.sh aws "$TARGET_BRANCH") && exit 0 || {
          log "Deploy failed; will retry after sleep"
        }
      fi
      ;;
    gsm)
      if check_gsm 2>/dev/null; then
        ...
        (cd "$REPO_ROOT" && GITHUB_ISSUE_ID="$GITHUB_ISSUE_ID" ./scripts/direct-deploy.sh gsm "$TARGET_BRANCH") && exit 0 || {
          log "Deploy failed; will retry after sleep"
        }
      fi
      ;;
