...
phase_dry_run() {
  ...
  if [ "$DRY_RUN" = "false" ]; then
    ...
    if gsutil -m cp -r gs://nexusshield-backup/* gs://nexusshield-test-replica/ &>/dev/null; then
...
phase_live_failover() {
  ...
  for stage_index in "${!TRAFFIC_STAGES[@]}"; do
    ...
    if [ "$DRY_RUN" = "false" ]; then
      ...
      if gcloud compute backend-services update nexus-shield-backend \
        --global \
        --enable-cdn \
  ...
  if [ "$DRY_RUN" = "false" ]; then
    for service in backend frontend database api portal; do
      if gcloud run services describe "nexus-shield-$service" --platform managed --project "$GCP_PROJECT" &>/dev/null; then
        ((services_ok++))
