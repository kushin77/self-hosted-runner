...
test_no_credentials() {
  # Use git grep with explicit pathspec excludes to avoid false positives
  local excludes=(
    ':(exclude)scripts/automation/pmo/tests/*'
    ':(exclude)scripts/load_gsm_secrets.sh'
    ':(exclude)scripts/run_gcp_vault_import.sh'
    ':(exclude)scripts/ci/*'
  ...
  if git -C "${GIT_ROOT}" grep -n -q "GITHUB_TOKEN=" -- "${excludes[@]}" 2>/dev/null; then
    if [ "${DEBUG_CRED_CHECK:-0}" = "1" ]; then
      echo "[debug] GITHUB_TOKEN= matches:" >&2
      git -C "${GIT_ROOT}" grep -n "GITHUB_TOKEN=" -- "${excludes[@]}" 2>/dev/null || true
    fi
    return 1
  fi

  if git -C "${GIT_ROOT}" grep -n -q -E "aws_secret_access_key|AWS_SECRET_ACCESS_KEY" -- "${excludes[@]}" 2>/dev/null; then
    if [ "${DEBUG_CRED_CHECK:-0}" = "1" ]; then
      echo "[debug] AWS key matches:" >&2
      git -C "${GIT_ROOT}" grep -n -E "aws_secret_access_key|AWS_SECRET_ACCESS_KEY" -- "${excludes[@]}" 2>/dev/null || true
    fi
    return 1
  fi
