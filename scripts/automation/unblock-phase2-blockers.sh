...
unblock_vault_approle() {
  ...
  # Export Vault token (assumes REDACTED_VAULT_TKN is set)
  export REDACTED_VAULT_TKN="${REDACTED_VAULT_TKN:-}"
  
  if [[ -z "$REDACTED_VAULT_TKN" ]]; then
    print_error "REDACTED_VAULT_TKN not set"
    log_audit "vault_auth" "error" "REDACTED_VAULT_TKN not configured"
    return 0
  fi
  ...
  curl -s -X GET "$VAULT_ADDR/v1/auth/approle" \
    -H "X-Vault-Token: $REDACTED_VAULT_TKN" &>/dev/null && {
    ...
    curl -s -X POST "$VAULT_ADDR/v1/sys/auth/approle" \
      -H "X-Vault-Token: $REDACTED_VAULT_TKN" \
  ...
  for ROLE in "${ROLES[@]}"; do
    ...
    ROLE_ID=$(curl -s -X GET "$VAULT_ADDR/v1/auth/approle/role/$ROLE/role-id" \
      -H "X-Vault-Token: $REDACTED_VAULT_TKN" | jq -r '.data.role_id' 2>/dev/null)
