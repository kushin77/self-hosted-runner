...
export class CredentialService {
  ...
  constructor() {
    ...
    const vaultTokenFile = process.env.VAULT_TKN_MOUNT_PATH || process.env.VAULT_TKN_FILE || '/var/run/secrets/vault/token';
    let vaultToken = process.env.REDACTED_VAULT_TKN || process.env.VAULT_TKN || '';
    try {
      const fs = require('fs');
