...
```bash
# If GCP_SERVICE_ACCOUNT_KEY updated, also update:
...
jq -r '.client_email' key.json  # Compare to GCP_SERVICE_ACCOUNT_EMAIL
```
...
- Updated GitHub secret GCP_SERVICE_ACCOUNT_KEY
...
gh secret set GCP_SERVICE_ACCOUNT_KEY < key-new.json
...
**Symptoms:**
```
Found secret pattern in commit: GCP_SERVICE_ACCOUNT_KEY
```
