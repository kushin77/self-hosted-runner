...
module "iam" {
  ...
  enable_workload_identity = var.enable_wif
}
...
module "cloud_run" {
  ...
  uptime_token_secret_name = var.uptime_token_secret_name
}

# Create a dedicated service account for audit purposes (used by compliance module)
resource "google_service_account" "cloud_audit" {
  account_id   = "cloud-audit"
  display_name = "Cloud Audit Service Account"
  project      = var.project_id
}

# COMPLIANCE MODULE: grant `roles/browser` to the cloud-audit service account
module "compliance" {
  ...
  # member should be a principal string supported by IAM, e.g. serviceAccount:...
  audit_member = "serviceAccount:${google_service_account.cloud_audit.email}"
}
